...
def plot_charge(
    ...
    for idx, fname in enumerate(files):
        ...
        if not idx:
            ...
            cell = res.get('cell')
            delta_tau = np.zeros(
                (nr_unit_cells*positions.shape[0], positions.shape[1]))
    ...
    # make a white figure
    figure = mlab.figure(1, bgcolor=(1, 1, 1), size=size)
    ...
    if take_difference:
        for f in forces[1:]:
            try:
                ...
                if shapediff > 0:
                    ff = np.concatenate((forces[0], np.zeros(
                        (forces[0].shape[0], shapediff))), axis=1)
                    return ff - forces[1]
                else:
                    raise Exception()
    else:
...
if __name__ == '__main__':
    ...
    p.add_argument('-f', '--forces', type=str,
                   nargs='+', help='some force files')
    p.add_argument('--fscale', type=float, default=1.)
    p.add_argument('--fstretch', type=float, default=1.)
    p.add_argument('--flog', action='store_true',
                   help='logarithmize the forces')
